...
def charge_battery():
    ...
    if m_running:
        ...
        measure_and_log_soc(True, voltage)
        print('')
...
def discharge_battery():
    global m_running, m_set_ovp, m_actual_capacity
    ...
    if m_running:
        m_running = False
        time.sleep(m_polling_delay)
        measure_and_log_soc(False, voltage, decay_time=m_decay_time)
        print('')
...
def open_file(charge=True):
    ...
    if charge:
        f.write('soc (%),soc (mW/h),clut (mV)\n')
    else:
        f.write('soc (%),soc (mW/h),llut (mV),hlut (mV)\n')
    return f
...
def measure_battery_voltage(charge=True, decay_time=0):
    with m_pause_lock:
        ...
        #initial_voltage = get_battery_voltage(Tenma.CHANNEL_1, discharge=not charge)
        ...
        print('{:.3f} V, resume (dis)charging'.format(voltage))
...
def measure_and_log_soc(charge, voltage=0, prev_clock=None, decay_time=0):
    if prev_clock is not None:
        print('')
        print('capacity {:.0f} mW/h ({:.1f} %), pausing for {} seconds'
              .format(m_actual_capacity * 1000, 100 * m_actual_capacity / m_total_capacity_w, m_decay_time))

    prev_voltage = voltage
    prev_capacity = m_actual_capacity
...
def profile_battery(charge=True):
    global m_lut_file, m_actual_capacity, m_total_capacity_w
    ...
    m_lut_file = open_file(charge)

    # measure state-of-charge and creat the look-up table
    m_total_capacity_w = m_total_capacity * m_nominal_voltage  # in W/h
    delta_capacity = m_total_capacity_w / m_lut_steps  # measure every 5 % increase
    m_actual_capacity = 0.0
    actual_clock = 0.0
    ...
    if charge:
        ...
        measure_and_log_soc(charge)
    else:
        charger = threading.Thread(target=discharge_battery)
        voltage, start_clock = measure_and_log_soc(charge)
    charger.start()
    ...
    while m_running:
        ...
        current = m_tenma.get_actual_current(Tenma.CHANNEL_1)
        clock = time.perf_counter()
        m_actual_capacity += voltage * current * (clock - prev_clock) / 3600
        actual_clock += clock - prev_clock
        prev_clock = clock
        print('{:.3f} V, {:.3f} A, {:.0f} mW/h ({:.0f} mW/h), {:.1f} %, {:.0f} s ({:.0f} s)'
              .format(voltage, current, m_actual_capacity * 1000, m_total_capacity_w * 1000,
                      100 * m_actual_capacity / m_total_capacity_w, actual_clock, clock - start_clock), end='\r')

        if m_actual_capacity - prev_capacity >= delta_capacity:
            prev_capacity, prev_clock = measure_and_log_soc(charge, voltage, prev_clock, decay_time=m_decay_time)
